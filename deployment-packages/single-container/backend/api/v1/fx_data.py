...
async def get_fx_data(
    ...
    """
    # 默认查询时间范围根据周期调整（支持4年历史数据）
    if not start_date:
        if period == "daily":
            start_date = date.today() - timedelta(days=1460)  # 约4年(1000个交易日)
        elif period == "weekly":
            start_date = date.today() - timedelta(days=2080)  # 约4年(208周)
        elif period == "monthly":
            start_date = date.today() - timedelta(days=1920)  # 约4年(48月)
        else:
            start_date = date.today() - timedelta(days=1460)
...
async def get_indicators(
    ...
    支持周期切换：daily（日线）/ weekly（周线）/ monthly（月线）
    """
    # 默认查询时间范围根据周期调整（技术指标需要足够数据，支持4年历史）
    if not start_date:
        if period == "daily":
            start_date = date.today() - timedelta(days=1500)  # 约4年+MA计算缓冲
        elif period == "weekly":
            start_date = date.today() - timedelta(days=2200)  # 约4年+缓冲
        elif period == "monthly":
            start_date = date.today() - timedelta(days=2100)  # 约4年+缓冲
        else:
    ...
    raw_data = await forex_daily_service.get_forex_daily_asc(
        ...
        start_date=start_date,
        end_date=end_date,
        limit=1500,  # 获取足够数据用于聚合和指标计算（支持4年历史）
    )
    ...
    result = aggregation_service.aggregate_with_indicators(
        daily_items,
        period,
        ma_periods=[5, 10, 20, 30, 60, 120, 240],  # 支持7条均线
