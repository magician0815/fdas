...
class PeriodAggregationService:
    ...
    def aggregate_with_indicators(
        ...
        for period in ma_periods:
            ...
            if total_len < period:
                # 数据不足，返回与输入长度一致的数组
                ma_data[key] = [{"value": None}] * total_len
                continue

            # 计算第一个有效MA值
            first_ma = sum(closes[:period]) / period

            # 前period-1个元素用第一个MA值填充（保证长度对齐）
            ma_values.extend([{"value": round(first_ma, 4)}] * (period - 1))

            # 从period-1开始计算后续MA值（总共total_len个元素）
        ...
        if len(closes) < slow + signal:
            # 数据不足，返回空数组但长度与closes一致（用None填充）
            macd_data = {
                "dif": [None] * len(closes),
                "dea": [None] * len(closes),
            ...
            # 使用公共EMA函数计算MACD（EMA现在返回与closes长度一致的数组）
            ema_fast = calculate_ema(closes, fast)
            ema_slow = calculate_ema(closes, slow)

            # DIF = EMA(fast) - EMA(slow)，长度与closes一致
            dif_values = []
            for i in range(len(closes)):
                dif = ema_fast[i] - ema_slow[i]
                dif_values.append(round(dif, 4))

            # DEA = EMA(DIF, signal)，长度与dif_values一致
            ...
            # 对齐数据长度：前面用None填充，使MACD数据与K线数据长度一致
            # MACD有效数据从第 slow + signal - 1 条开始（即第34条）
