...
describe('ChartConfig 图表配置工具', () => {
  ...
  describe('getKLineBaseOption K线基础配置', () => {
    ...
    it('应包含三个grid区域（主图+成交量+MACD）', () => {
      ...
      expect(option.grid[0].height).toBe('70%') // 主图
      expect(option.grid[1].height).toBe('10%') // 成交量
      expect(option.grid[2].height).toBe('8%')  // MACD
    })
  ...
  describe('getKLineSeriesOption K线系列配置', () => {
    ...
    it('K线宽度应设置为50%', () => {
      const series = getKLineSeriesOption('light')
      expect(series.barWidth).toBe('50%')
    })
  ...
  describe('getMASeriesOption 均线系列配置', () => {
    ...
    it('MA30配置应正确', () => {
      ...
      expect(series.name).toBe('MA30')
      expect(series.lineStyle.color).toBe('#ec4899')
    })
