    ...
    <div class="datasource-grid">
      <div class="datasource-card" v-for="ds in datasources" :key="ds.id">
        ...
        <div class="card-footer">
          ...
          <el-button size="small" type="danger" @click="confirmDeleteDatasource(ds)" :loading="ds.deleting">
    ...
    >
      <div class="config-editor">
        <el-form-item label="数据源名称">
          <el-input v-model="configName" placeholder="请输入数据源名称" />
        </el-form-item>
        <el-alert
          title="配置说明"
          type="info"
      ...
      <div class="import-form">
        <el-form :model="importForm" label-width="100px">
          <el-form-item label="数据源类型" required>
            <el-select v-model="importForm.type" placeholder="请选择数据类型" @change="handleTypeChange">
              <el-option label="外汇" value="forex" />
              <el-option label="A股" value="stock_cn" />
            </el-select>
          </el-form-item>
...
import { Connection, Refresh, CircleCheck, Edit, Setting, Download, Upload, Delete } from '@element-plus/icons-vue'
...
// 根据ID获取市场代码
const getMarketCode = (marketId) => {
  const m = markets.value.find(m => m.id === marketId)
  return m?.code || 'forex'
}

// 获取A股默认配置
const getAStockConfig = () => {
  return JSON.stringify({
    version: "1.0",
    name: "东方财富A股数据源",
    type: "akshare",
    market: "stock_cn",
    collector: "akshare_stock",
    symbol_mapping: {
      "sh600519": "贵州茅台",
      "sh600036": "招商银行",
      "sh600900": "长江电力",
...
const handleTypeChange = (type) => {
  if (type === 'stock_cn') {
    // A股市场
    importForm.config_file = getAStockConfig()
    // 查找A股市场ID
    const aStock = markets.value.find(m => m.code === 'stock_cn')
    if (aStock) {
      importForm.market_id = aStock.id
    }
  } else {
    // 外汇市场
    importForm.config_file = getDefaultConfig()
    const forex = markets.value.find(m => m.code === 'forex')
    if (forex) {
      importForm.market_id = forex.id
    }
  }
}
...
const showConfigDialog = async (ds) => {
  try {
    ...
    configName.value = ds.name
    const res = await getDatasourceConfig(ds.id)
    if (res.success) {
      configContent.value = res.data.config_file || ''
      if (!configContent.value) {
        // 根据数据源的市场类型获取默认配置
        const marketCode = getMarketCode(ds.market_id)
        configContent.value = getDefaultConfig(marketCode)
      }
    } else {
...
const saveConfig = async () => {
  ...
  try {
    ...
    // 如果名称改变了，先更新名称
    if (configName.value !== currentDatasource.value.name) {
      const nameRes = await updateDatasourceName(
        currentDatasource.value.id,
        configName.value.trim()
      )
      if (!nameRes.success) {
        ElMessage.error(nameRes.message || '名称更新失败')
        return
      }
    }
...
const resetConfig = () => {
  if (currentDatasource.value) {
    const marketCode = getMarketCode(currentDatasource.value.market_id)
    const marketName = getMarketName(currentDatasource.value.market_id)
    configContent.value = getDefaultConfig(marketCode)
    ElMessage.info(`已重置为${marketName}默认配置`)
  } else {
...
// 删除数据源
const deleteDatasourceById = async (ds) => {
  try {
    ds.deleting = true
    const res = await deleteDatasource(ds.id)
    if (res.success) {
