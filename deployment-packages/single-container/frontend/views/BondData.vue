    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div class="symbol-info">
          ...
          <span class="yield-value" v-if="currentYield">收益率: {{ currentYield }}</span>
        </div>
      </div>
...
import { getBondDailyData } from '@/api/bond_data'
...
const periodType = ref<string>(
  localStorage.getItem('fdas_bond_period_type') || 'daily'
)
...
const handlePeriodChange = (period: string) => {
  ...
  localStorage.setItem('fdas_bond_period_type', period)
  fetchData()
}
...
const currentYield = computed(() => {
  if (chartData.value.length) {
    const latest = chartData.value[chartData.value.length - 1]
    return formatYield(latest.yield)
  }
  return null
})
...
const handleSymbolSearch = async (query: string) => {
  ...
  try {
    const res = await getBondSymbols({ search: query })
    if (res.success) {
...
const fetchData = async () => {
  ...
  try {
    // 获取债券行情数据
    const dataRes = await getBondDailyData({
