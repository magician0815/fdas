...
describe('KLineChart', () => {
  ...
  describe('组件渲染', () => {
    ...
    it('应该渲染工具栏左侧标题区域', () => {
      ...
      expect(wrapper.find('.chart-title').text()).toBe('行情走势')
      expect(wrapper.find('.symbol-name-tag').exists()).toBe(true)
      expect(wrapper.find('.symbol-name-tag').text()).toBe('USDCNH')
    })
