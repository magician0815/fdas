...
describe('Auth Store', () => {
  ...
  describe('login函数', () => {
    it('登录成功应返回success:true并设置user', async () => {
      ...
      // axios拦截器会返回response.data，所以mock需要返回{data: {user, session_id}}
      axios.post.mockResolvedValue({ data: { user: mockUser, session_id: 'test-session' } })
    ...
    it('登录成功后isAdmin应根据role判断', async () => {
      // admin角色
      axios.post.mockResolvedValue({ data: { user: { role: 'admin' }, session_id: 'test-session' } })
      ...
      // user角色
      axios.post.mockResolvedValue({ data: { user: { role: 'user' }, session_id: 'test-session' } })
  ...
  describe('状态变化', () => {
    it('登录后再登出应清除所有状态', async () => {
      axios.post.mockResolvedValue({ data: { user: { id: '1', role: 'admin' }, session_id: 'test-session' } })
    ...
    it('连续登录应更新user', async () => {
      axios.post.mockResolvedValue({ data: { user: { id: '1', username: 'user1' }, session_id: 'test-session' } })

      const store = useAuthStore()
      await store.login('user1', 'password')
      expect(store.user.username).toBe('user1')

      axios.post.mockResolvedValue({ data: { user: { id: '2', username: 'user2' }, session_id: 'test-session' } })
